...
export default {
  ...
  methods: {
    ...
    judgeUser: function() {
      ...
      if (changeTitle) me.$store.dispatch("ChangeRenderFunc", { key: 'changeTitle', value: changeTitle });

      let changeSubtitle = me.$route.query.changeSubtitle;
      if (changeSubtitle) me.$store.dispatch("ChangeRenderFunc", { key: 'changeSubtitle', value: changeSubtitle });

      let changeLogo = me.$route.query.changeLogo;
      if (changeLogo) me.$store.dispatch("ChangeRenderFunc", { key: 'changeLogo', value: changeLogo });

      let changeMain = me.$route.query.changeMain;
      if (changeMain) me.$store.dispatch("ChangeRenderFunc", { key: 'changeMain', value: changeMain });

      let logoFileId = me.$route.query.logoFileId;
      if (logoFileId) me.$store.dispatch("ChangeRenderFunc", { key: 'logoFileId', value: logoFileId });

      let mainFileId = me.$route.query.mainFileId;
      if (mainFileId) me.$store.dispatch("ChangeRenderFunc", { key: 'mainFileId', value: mainFileId });
    },
  },
  components: {
