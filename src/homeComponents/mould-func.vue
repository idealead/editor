  ...
  <div class="shadow">
    <div class="setTextPositionBlock" v-if="showText">
      ...
      <div class="textSwitch">
        <el-switch v-bind:checked="saveTextRule" active-text="上传规则" inactive-text="不上传规则" v-on:change="change" v-model="textval">
        </el-switch>
      </div>
      <el-tag :key="tag" v-for="tag in textRepalceF.textPositionArr" closable :disable-transitions="false" @close="handleClose(tag)">
        {{tag}}
      </el-tag>
      <el-input class="input-new-tag" v-if="textRepalceF.inputVisible" v-model="textRepalceF.inputValue" ref="saveTagInput" size="small" @keyup.enter.native="handleInputConfirm"
...
export default {
  ...
  mounted: function() {
    ...
    if (me.openBlock === "text") {
      ...
      me.$set(
        me.textRepalceF,
        "textPositionArr",
        _.cloneDeep(me.textPositionArr)
      );
      // $set是浅拷贝，
    }
  },
  methods: {
    ...
    handleClose(tag) {
      this.textRepalceF.textPositionArr.splice(
        this.textRepalceF.textPositionArr.indexOf(tag),
        1
      );
    },
    showInput() {
