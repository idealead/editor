<template>
  <div id="mainCanvas">
    <div class="mouse_right_block" v-show="right_block.show" v-bind:style="{ left: right_block.rb_left, top: right_block.rb_top }">
      <div class="mouse_right_btn right_btn_association" v-show="right_block_ass && user_type != 'designer'">
      ...
      <div v-if="new_structure_id != ''" class="mouse_right_btn" @click="moveIndex(true, 'maxUp')">置顶</div>
      <div v-if="new_structure_id != ''" class="mouse_right_btn" @click="moveIndex(false, 'maxDown')">置底</div>
      <div class="mouse_right_btn" @click="onEdit" v-show="right_block.menu.edit_text">更改文字</div>
      <div class="mouse_right_btn" @click="changeImage" v-show="right_block.menu.change">更换图片</div>
      <div class="mouse_right_btn" @click="toImage(false)">导出图片</div>
      <div class="mouse_right_btn right_btn_delete" v-show="right_block.menu.del && in_move && !in_move.structure" @click="onRightDelete">删除</div>
      <div v-if="user_type == 'designer'">
        ...
        <div class="mouse_right_btn" @click="setRole('subtitle')" v-show="
          ...
          ">设为副标题</div>
        <div class="mouse_right_btn" @click="setRole('arc_title')" v-show="
            in_move && in_move.type == 'text' && in_move.role != 'arc_title'
          ">设为弧形标题</div>
        <div class="mouse_right_btn" @click="setRole('arc_subtitle')" v-show="
            in_move && in_move.type == 'text' && in_move.role != 'arc_subtitle'
          ">设为弧形副标题</div>
        <div class="mouse_right_btn" @click="setRole('logo')" v-show="in_move && in_move.type == 'image' && in_move.role != 'logo'">设为logo</div>
        <div class="mouse_right_btn" @click="setRole('main')" v-show="in_move && in_move.type == 'image' && in_move.role != 'main'">设为主图</div>
        <div class="mouse_right_btn" @click="setRole('arc_titleImg')" v-show="
            in_move && in_move.type == 'image' && in_move.role != 'arc_titleImg'
          ">设为弧形标题装饰</div>
        <div class="mouse_right_btn" @click="setRole('arc_subtitleImg')" v-show="
            in_move &&
              in_move.type == 'image' &&
        ...
        <div v-show="
            in_move &&
              ...
              new_structure_id != ''
          " v-for="(item, index) in structure_position" :key="index" class="mouse_right_btn" @click="setPosition(item.position)">"{{ item.position_name }}"替换位置</div>
        <div class="mouse_right_btn" @click="setPosition(null)" v-if="
            in_move &&
              in_move.type == 'image' &&
          ...
          ">取消替换位置</div>
        <div class="mouse_right_btn" @click="setRole('bg')" v-show="
            in_move &&
              in_move.type == 'image' &&
              in_move.role != 'bg' &&
              tempRoleMsg.bg == 0
          ">设为背景</div>
        <div class="mouse_right_btn" @click="setRole('normal')" v-show="in_move && in_move.role != 'normal'">取消角色</div>
      </div>
    </div>
    <transition name="slide-fade">
      <div class="edit_bar" v-show="edit_bar.btn.show">
        <el-tooltip class="item" effect="light" content="编辑" placement="right" popper-class="tooltip">
          <div class="edit_btn editf_btn" v-show="edit_bar.btn.edit" @click="onEdit"></div>
        </el-tooltip>
        <el-tooltip class="item" effect="light" content="编辑文字" placement="right" popper-class="tooltip">
          <div class="edit_btn text_btn" v-show="edit_bar.btn.text" @click.self="show_text_block" :class="{ text_btn_active: edit_bar.edit_text }">
            <div class="text_block" v-show="edit_bar.edit_text">
              ...
              <div class="text_body_block">
                <el-row class="text_row">
                  <el-col :span="24">
                    ...
                    <el-button class="text_fontFamily" @click="showFontFamily">{{ edit_bar.text.fontFamily }}
                    </el-button>
                  </el-col>
                </el-row>
                <el-row class="text_row">
                  <el-col :span="24">
                    ...
                    <el-input v-model="edit_bar.text.fontSize" @change="fontSize_change" class="text_fontSize align_right" type="number"></el-input>
                  </el-col>
                </el-row>
                <el-row class="text_row">
                  <el-col :span="24">
                    <div class="grid-content text_left">
                      倾斜
                      <el-switch class="align_right" :value="edit_bar.text.fontStyle == 'normal' ? false : true" active-color="#13ce66" inactive-color="#ff4949" @change="fontStyle_change"></el-switch>
                    </div>
                  </el-col>
                </el-row>
                <el-row class="text_row">
                ...
                <el-row class="text_row" style="padding-bottom:0">
                  <el-col :span="24">
                    <div class="grid-content text_left">
                      字间距
                      <el-slider v-model="edit_bar.text.letterSpacing" class :min="-20" :max="50" @change="letterSpacing_change"></el-slider>
                    </div>
                  </el-col>
                </el-row>
                <el-row class="text_row" style="padding-bottom:0">
                  <el-col :span="24">
                    <div class="grid-content text_left">
                      行间距
                      <el-slider v-model="edit_bar.text.leading" class :min="-20" :max="50" @change="leading_change">
                      </el-slider>
                    </div>
                  </el-col>
                </el-row>
              ...
              <div class="fontFamily_select" v-show="edit_bar.edit_fontFamily_select">
                ...
                <div class="fontFamily_body_block">
                  <div class="fontFamily_option" v-for="item in edit_bar.fontFamily" :key="item.value" @click="fontFamily_change(item.value)" :style="{ 'font-family': item.value }">{{ item.name }}
                  </div>
                </div>
              </div>
            </div>
        ...
        <el-tooltip class="item" effect="light" content="颜色" placement="right" popper-class="tooltip">
          <div class="edit_btn color_btn" v-show="edit_bar.btn.color" @click.self="showColorBlock" :class="{ color_btn_active: edit_bar.edit_color }">
                ...
                <el-row class>
                  <el-col :span="24">
                    <div class="block text_left">
                      <el-color-picker class="align_right font_color_picker" v-model="edit_bar.text.color" @active-change="fontColor_change" popper-class="font_color"></el-color-picker>
                    </div>
                  </el-col>
                </el-row>
              </div>
        ...
        <el-tooltip class="item" effect="light" content="对齐" placement="right" popper-class="tooltip">
          <div class="edit_btn align_btn" v-show="edit_bar.btn.align" @click.self="showEditAlign" :class="{ align_btn_active: edit_bar.edit_align }">
        ...
        <el-tooltip class="item" effect="light" content="阴影" placement="right" popper-class="tooltip">
          <div class="edit_btn shadow_btn" v-show="edit_bar.btn.shadow" @click.self="showEditShadow" :class="{ shadow_btn_active: edit_bar.edit_shadow }">
              ...
              <div class="shadow_body_block">
                <el-row class="text_row">
                  <el-col :span="24">
                    <div class="grid-content text_left">
                      开启阴影
                      <el-switch class="align_right" v-model="edit_bar.text.dropShadow" active-color="#13ce66" inactive-color="#ff4949" @change="dropShadow"></el-switch>
                    </div>
                  </el-col>
                </el-row>
                <div v-show="edit_bar.text.dropShadow">
                  <el-row class="text_row">
                    <el-col :span="24">
                      <div class="block text_left">
                        阴影颜色
                        <el-color-picker color-format="hex" class="align_right shadow_color_picker" v-model="edit_bar.text.dropShadowColor" @active-change="dropShadowColorChange" size="small"
                          popper-class="font_color shadow_color"></el-color-picker>
                      </div>
                    </el-col>
                  </el-row>
                  <el-row class="text_row">
                    <el-col :span="24">
                      <div class="grid-content text_left">
                        阴影模糊
                        <el-slider v-model="edit_bar.text.dropShadowBlur" class :min="0" :max="20" @change="dropShadowBlurChange"></el-slider>
                      </div>
                    </el-col>
                  </el-row>
                  <el-row class="text_row">
                    <el-col :span="24">
                      <div class="grid-content text_left">
                        阴影距离
                        <el-slider v-model="edit_bar.text.dropShadowDistance" class :min="0" :max="30" @change="dropShadowDistanceChange"></el-slider>
                      </div>
                    </el-col>
                  </el-row>
                </div>
        ...
        <el-tooltip class="item" effect="light" content="更换图片" placement="right" popper-class="tooltip">
          <div class="edit_btn changef_btn" v-show="edit_bar.btn.change_image" @click="changeImage"></div>
        </el-tooltip>
        <el-tooltip class="item" effect="light" content="位置" placement="right" popper-class="tooltip">
          <div class="edit_btn location_btn" v-show="edit_bar.btn.location" @click.self="showEditLocation" :class="{ location_btn_active: edit_bar.edit_location }">
        ...
        <el-tooltip class="item" effect="light" content="旋转" placement="right" popper-class="tooltip">
          <div class="edit_btn rotation_btn" v-show="edit_bar.btn.rotation" @click.self="show_rotation_block" :class="{ rotation_btn_active: edit_bar.edit_rotation }">
            <div class="rotation_block" v-show="edit_bar.edit_rotation">
              <div class="block_title">旋 转</div>
              <div class="rotation_body_block">
                <el-slider v-model="edit_bar.rotation" @change="rotationChange" :min="-180" :max="180" :show-tooltip="false"></el-slider>
          ...
          <div class="edit_btn opacity_btn" v-show="edit_bar.btn.opacity" @click.self="show_opacity_block" :class="{ opacity_btn_active: edit_bar.edit_opacity }">
            <div class="opacity_block" v-show="edit_bar.edit_opacity">
              ...
              <div class="opacity_body_block">
                <el-slider v-model="edit_bar.opacity" @change="opacity_change" class="opacity_slider" :show-tooltip="false"></el-slider>
                <p>{{ edit_bar.opacity }}%</p>
              </div>
            </div>
          </div>
        </el-tooltip>
        <el-tooltip class="item" effect="light" content="层级" placement="right" popper-class="tooltip">
          <div class="edit_btn upDown_btn" v-show="edit_bar.btn.updown" @click="showLayerBlock" :class="{ upDown_btn_active: edit_bar.edit_layer }">
        ...
        <el-tooltip class="item" effect="light" content="删除" placement="right" popper-class="tooltip">
          <div class="edit_btn delete_btn" v-show="edit_bar.btn.delete" @click="onRightDelete"></div>
        </el-tooltip>
      </div>
    </transition>
    <div class="addButton" @click="addPopShow(true)"></div>
    <div class="hueButton">
      <div class="hue_block">
...
export default {
    ...
    return {
      ...
      edit_bar: {
        ...
        fontFamily: [
          ...
          // { name: "汉鼎繁古印", value: "hdfguyin1b86cafc8f206a3" },
          // { name: "方正正粗黑体", value: "FZZhengHeiS" },
  ...
  beforeDestroy: function() {
    ...
    //尝试解决，内存泄露
    document.getElementById("mainCanvas").removeChild(me.p_app.view);
    me.key_zf = null;
    me.key_yf = null;
  ...
  methods: {
    allHue: function(value) {
      ...
      me.mainStage_container.filters = [filter];
    },
    ...
    mouldImg: function() {
      ...
      return me.ClippingImage(
        me.p_app.renderer.plugins.extract.base64(me.p_app.stage),
        me.canvas_width,
        me.canvas_height,
        0.92,
      ...
      return new Promise(function(resolve, reject) {
        ...
        if (last.children[0].ass_children.length == 1) {
          ...
          if (last.children[0].ass_children[0].type == "image") {
            ...
            material_children.file_id = me.json_f([
              parseInt(last.children[0].ass_children[0].file_id)
            ]);
            material_children.author = me.user_data.id;
            material_children.preview = parseInt(
              last.children[0].ass_children[0].file_id
            );
            ...
            material_children["content"] = me.json_f(last);
            material_children["text_content"] =
              last.children[0].ass_children[0].text;
          }
      ...
      axios({
        ...
        .then(function(response) {
          if (me.user_type == "designer") {
            ...
            me.$store.dispatch(
              "changeTempIdFunc",
              parseInt(response.data.templateId)
            );
            bus.$emit("clientSave");
            if (funcData.leave) {
              //跳转
      ...
      if (me.tempId) {
        ...
        axios({
          ...
          .then(function(response) {
            ...
            for (
              let i = 0;
              i < response.data.data.material_content.length;
              i++
            ) {
              if (response.data.data.material_content[i].content) {
                render_data.push(
                  me.obj_f(response.data.data.material_content[i].content)
                );
              }
            }
            ...
            //设置模板宽高，名字
            me.$store.dispatch("setCanvasFunc", [
              response.data.data.template_width,
              response.data.data.template_height
            ]);
    ...
    show_rotation_block: function() {
      ...
      if (me.in_move && !me.temporary_rect) {
        me.$set(
          me.edit_bar,
          "rotation",
          Math.floor((me.in_move.rotation * 180) / Math.PI)
        );
      } else if (me.temporary_rect) {
        me.$set(
          me.edit_bar,
          "rotation",
          Math.floor((me.temporary_rect.rotation * 180) / Math.PI)
        );
      }
    ...
    rotationChange: function(value) {
      ...
      if (me.in_move && !me.temporary_rect) {
        ...
        me.outLine_container.getChildByName(me.in_move.name).rotation =
          (value * Math.PI) / 180;
        setTimeout(() => {
    ...
    findFinalIndex: function(m_comp_name) {
      ...
      if (me.container_arr.length == 0) {
        ...
        me.project_m_comp[0].name == m_comp_name &&
    ...
    createBorder: function(
      ...
      container_func.createBorder.bind(this)(
        obj,
        t_a,
        a_r_btn,
        show_btn,
    ...
    onAssociationStart: function(event, right = false, lock = false) {
      const me = this;
      element_func.onAssociationStart.bind(me)(event, right, lock);
    },
    ...
    createAssociation: function(name, m_comp_name, render_create) {
      container_func.createAssociation.bind(this)(
        name,
        m_comp_name,
        render_create
      );
    ...
    createAssociationRect: function(container, name, render_create) {
      container_func.createAssociationRect.bind(this)(
        container,
        name,
        render_create
      );
    ...
    changeInMoveImage: function(data) {
      ...
      for (let i = 0; i < last.length; i++) {
        if (last[i].children[0].ass_children.length == 1) {
          if (
            last[i].children[0].ass_children[0].type == "image" &&
            last[i].children[0].ass_children[0].file_id == me.in_move.file_id
          ) {
    ...
    renderARC: function(titleType, wordArr) {
      container_func.renderARC.bind(this)(titleType, wordArr);
    },
